...
def accuracy_test(rows, k):  # k=0 is 10, k=1 is 5, k=2 is 3
    ...
    for i in rows:
        try:
            # gets the predictions given x and ranks them on softmax probability
            ...
            # lists the top n predictions for 10 5 and 3
            top_ten_predictions = [str(classes[sorted_models[0]]), str(classes[sorted_models[1]]),
                                   str(classes[sorted_models[2]]), str(classes[sorted_models[3]]),
                                   str(classes[sorted_models[4]]), str(classes[sorted_models[5]]),
                                   str(classes[sorted_models[6]]), str(classes[sorted_models[7]]),
            ...
            prediction_models = [top_ten_predictions, top_five_predictions, top_three_predictions]

            # counter for the correct prediction
...
# runs this test for top 1, top 3, and top 5
accuracy_5 = accuracy_test(random.sample(range(130000), 1000), 1)
accuracy_3 = accuracy_test(random.sample(range(130000), 1000), 2)
print("The accuracy for the top five colors is: " + str(accuracy_5[0]))
print("The accuracy for the top three colors is: " + str(accuracy_3[0]))
print("The accuracy for the top color alone in: " + str(accuracy_5[1]))
